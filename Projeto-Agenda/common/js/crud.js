...
$.each(
    jsonDados, function(indice, contato) {
        tabela += '<tr>';
        ...
        tabela += '<td><a href="javascript:void(0);" id="editar_contato" contato_id=""' + contato.id +
        'class="btn btn-sucess btn-xs"><i class="glyphicon glyphicon-pencil"></i></a></td>';
        tabela += '<td><a href="javascript:void(0);" id="confirma_excluir" contato_id=""' + contato.id +
...
function getEditarContato(elemento) {
    var Contato = new Object();
    Contato.id = $(elemento).attr('contato.id');
...
function salvarConteudoEdicao() {
    $('#carregando').show();

    var Contato = new Object();
    Contato.id = $('input#contato_id').val();
    Contato.nome = $('input#contato.nome').val();
    Contato.telefone = $('input#contato.telefone').val();
    Contato.email = $('input#contato.email').val();
    Contato.endereco = $('textarea#contato.endereco').val();
